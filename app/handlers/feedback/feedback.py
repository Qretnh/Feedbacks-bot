...
from app.db.models.statistics import FeedbackStatistics
...
async def menu_feedback(
    ...
    current_page = data.get("feedback_page", 0)

    await callback.message.edit_text(
        i18n.feedback.choose(), reply_markup=topic_keyboard(topics, current_page)
    )
    await callback.answer()


@router.callback_query(F.data.startswith("feedback_next_page"))
async def feedback_next_page(
    callback: CallbackQuery,
    i18n: TranslatorRunner,
    session: AsyncSession,
    state: FSMContext,
    ...
    data = await state.get_data()
    current_page = data.get("feedback_page", 0)
    await state.update_data(feedback_page=current_page + 1)
    await menu_feedback(callback, i18n, state, session)


@router.callback_query(F.data.startswith("feedback_prev_page"))
async def feedback_prev_page(
    callback: CallbackQuery,
    i18n: TranslatorRunner,
    session: AsyncSession,
    state: FSMContext,
    ...
    data = await state.get_data()
    current_page = data.get("feedback_page", 0)
    await state.update_data(feedback_page=max(0, current_page - 1))
    await menu_feedback(callback, i18n, state, session)
...
async def want_comment(
    ...
    if choice == "yes":
        ...
        await update_statistics(session, FeedbackStatistics)
