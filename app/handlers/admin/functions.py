...
from app.keyboards.admin import (
    admin_menu_faq_keyboard,
    admin_menu_keyboard,
    admin_menu_topic_keyboard,
)
...
async def admin_callback(message: Message, i18n: TranslatorRunner, state: FSMContext):
    ...
    if message.from_user.id not in admin_ids:
        await message.answer(i18n.admin.permissions.error())
        return
    await message.answer(i18n.admin.menu(), reply_markup=admin_menu_keyboard)
...
async def admin_faq_menu(
    ...
    await callback.message.answer(
        ...
        reply_markup=admin_menu_faq_keyboard(questions),
    )
    await callback.answer()
...
async def admin_faq_menu(
    ...
    questions = await get_feedback_topics(session)
    await callback.message.answer(
        text=i18n.admin.choose.topic(),
        reply_markup=admin_menu_topic_keyboard(questions),
    )
    await callback.answer()
